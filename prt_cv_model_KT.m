...
%         stats_train = prt_stats(model, targets.train, nc); %targets.train
        
        % update PRT
        PRT.model(mid).output(k).fold(f).targets     = targets.test;
        PRT.model(mid).output(k).fold(f).targets_train     = targets.train;
        PRT.model(mid).output(k).fold(f).predictions = model.predictions(:);
        PRT.model(mid).output(k).fold(f).predictions_train = model.predictions_train(:);
        PRT.model(mid).output(k).fold(f).stats       = stats;
%         PRT.model(mid).output(k).fold(f).stats_train       = stats_train;
