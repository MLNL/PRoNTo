...
switch S.mode
    ...
    case 'channel x time x frequency'
        if ~isTF
            error('This mode is only supported for TF datasets.');
        end
        ...
        N.mat(2,4) = N.mat(2,4) - N.mat(2,1);
        N.mat(3,4) = N.mat(3,4) - N.mat(3,2);
        
        dat = file_array('', ...
            [length(chanind) length(freqind) length(timeind)], 'FLOAT32-LE');
