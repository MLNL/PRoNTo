...
\item[Model]: this panel allows the user to select the model to analyse, whether to analyse a
particular fold or all folds at once, and which plot to produce.
\item[Stats]: The {\tt stats} panel allows the user to generate a variety of statistics on the test (based on the selected fold), including accuracy statistics for classifiers and p-values on these parameters via permutation testing.
\end{description}
...
of the machine, and where modelling assumptions may prove false. In PRoNTo, plots are different for classification and regression.

\subsection{Classification}

\subsubsection{Predictions plot}

A prediction plot displays, for a particular fold, the output value of the machine's
...
departs from the training set. This plot is available for binary classification.
...
Figure \ref{fig_prt_ui_results_plots_hist} shows an example of such a plot for a multi-class classification (Haxby dataset, Faces versus Houses versus Scissors, LOBO CV).


\begin{figure}[h!]
\begin{center}
\includegraphics[width=8cm]{images/prt_ui_results_plots_hist.png}
\caption{Example function values histogram curve for a three-class problem modelled by GP.}
...
This plot will be present in the list if hyper-parameter optimization was performed. For each value of the hyper-parameter, it displays the average model performance (balanced accuracy for classification and MSE for regression) across nested folds, with an error bar representing the standard deviation of model performance. The frequency of selection of a hyper-parameter value (i.e. the number of times this value was returned as `optimal' to the outer CV fold) is represented using a colorscale. An example of such a plot is displayed in Figure \ref{fig_prt_ui_results_plots_hyper} for the optimization of the soft-margin parameter in L1-MKL (Haxby dataset, Faces versus Houses, 4-folds nested CV, LOBO outer CV). 

\begin{figure}[h!]
\begin{center}
\includegraphics[height=6cm]{images/prt_ui_results_plots_hyper.png}
\caption{Example performance curve depending on the hyper-parameter value.}
...
acc = 1- \frac{1}{N} \sum_n l_{01}(\omega_n,f(\mathbf{x}_n)),
\end{equation}
...
equivalent to sensitivity.In the same way, the figure displays class predictive value, which represents the number of false positives for each class. An example of classification stats is displayed in Figure \ref{fig_prt_ui_results_statsTable}.
...
field with a number of repetitions $R$, pressing the {\tt Permutation test} button will run for the specified number of times, and produce a  p-value for performance statistics (see Figure~\ref{fig_prt_ui_results_statsTable}). The smallest increment in p-value is proportional to $1/R$ (e.g. 20 permutations gives you increments of 0.05), with a minimum value of $1/R$ (i.e. running 10 permutations will never lead to statistically significant result at the commonly used threshold of $p<0.05$). Usually, we would recommend computing several hundreds to a thousand permutations.
