...
\end{figure}
	...
	\item Finally, load all the image files available in the fMRI directory (Haxby\_dataset/fMRI/). You can select all the files by using the right mouse button and clicking on the option `Select All' (Figure \ref{fig:files}). When all the images are selected, click on `Done' button;
...
\end{figure}	
	...
	\item The `Data and design' window should look similar to the Figure \ref{fig:finalDesign}. Click on `Save' button to create `PRT.mat' file with the structure containing the information that has been previously specified. If no errors are shown in the MATLAB command, leave the `Data and design' window by clicking `Quit'.
...
\end{figure}
	...
	\begin{itemize}
		...
		\item Features box: leave the additional mask field as it is and the `build one kernel per region' tick box unchecked. Then, click on `Done' button.  
...
\end{figure}
	...
	\item In the `Data operations' box, select the `Sample averaging (within block)' option, which corresponds to a temporal compression of the data within each block, and `Mean centre features using training data' option. Then, the `Specify model' window should look similar to the Figure \ref{fig:finalModel};
...
\end{figure}
	...
	\item In the `Model' panel, select the model that you want to view, `svmFacesHouses', and the results through performance will be similar to the Figure \ref{fig:stats};
...
\end{figure}
	...
	\item Finally, to check the significance of the results, run a permutation test  by clicking on `Permutation test' button with 100 repetitions. Results will be displayed on the MATLAB prompt (Figure \ref{fig:permutations}).
...
\end{figure}
	...
	\item By clicking on `Model', svmFacesHouses, an image will appear in the `Weights map' box; and to show the `Anatomical img' you have to load an anatomical image for reference. A template image can be found in the SPM8's canonical folder (`single\_subj\_T1` file). The final result window will look similar to that shown in the Figure \ref{fig:modelInterpretation}.
...
\begin{itemize}
	...
	\item In the `Groups' field:
		...
		\item In the `Data \& Design' field, choose `Load SPM.mat' option. This file is available with the Haxby dataset on PRoNTo's website\footnote{http://www.mlnl.cs.ucl.ac.uk/pronto/prtdata.html} inside the folder Haxby\_dataset/design/. The batch editor should look similar to the Figure \ref{fig:batchGroups};	
...
\end{figure}
	...
	\item With `Load PRT.mat' field selected, click on `Dependency' button to associate the `PRT.mat' file created in the previous `Data \& Design' step (Figure \ref{fig:batchDependency}) or click on `Select files' button to browse where `PRT.mat' file was saved;
...
\end{figure}
	...
	\item Leave `Load Atlas' as default. After all this steps, the batch editor should look similar to the Figure \ref{fig:batchModality};
...
\end{figure}
	...
	\item Select the `Classification' model type:
		...
		\item After all this steps, the batch editor should look similar to the Figure \ref{fig:batchClass}	
...
\end{figure}
	...
	\begin{itemize}
	\item Select `SVM Classification' option;	
...
\begin{itemize}
	...
	\item Click on `Run model' option on PRoNTo's {\tt matlabbatch} menu (Figure \ref{fig:batchRun});
...
\begin{itemize}
	...
	\item Click on `Compute weights' option on PRoNTo's {\tt matlabbatch} menu (Figure \ref{fig:batchWeights});
...
Finally, save the batch (e.g. as batch\_run\_all.m) and click on `Run Batch' option, in the `File menu'. The batch file created can then be opened and edited for further analyses. The results will be the same as those obtained using the GUI (see Section \ref{display_results}  of this chapter).
