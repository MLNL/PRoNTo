...
\paragraph{Select by scans} Here the data is assumed to have been acquired without an experimental design, and therefore the `No design' option is automatically selected and cannot be changed (bottom window in Figure \ref{Fig2.3}). However, in select by scans, the user can also introduce `Covariates', i.e. a variable that covaries with the data (subjects) but of no interest to the subsequent analyses. This option will be functional in version 2.1 of PRoNTo. It requires the input of a matrix, with one row per image/subject. This matrix can either be entered as a Matlab command in the editable box, or the full path to a .mat containing the matrix should be provided (matrix named  `R'). This last option is recommended to input a matrix (i.e. more than one covariate). The order of covariates and corresponding imaging files is essential, hence users shall be careful about it. 

In v2.1, we have added removing confounds. Before input covariate values to PRoNTo, we recommend users to firstly check that their covariates do not correlate with the targets \cite{Miller2001}. If users have categorical covariates, such as genders, scanner centres, handness et al., we recommend one-hot encoding before constructing the covariate matrix. The reason is for that machines in PRoNTo process numerical inputs, hence users need to convert the categorical data to numerical values. However, if there is no order in the categorical data, we code them using 0, 1, 2, 3..., we implicitly introduce ordinal structure in the coded covariate, which may be misinterpreted by the machines. In other words, machines may interpret categorical data with no ordinal information as ordinal data because of the way we convert them. For instance, if there are K scanner centres and we want to remove their effects from the data. There is no ordinal information in the centre IDs. However, when we put them as one numerical column in the R matrix like [1,2,..,K], the machine we use later may see this covariate as ordinal data. This may introduce extra confounds and even mistakes in the final results. One-hot encoding is a way to solve this problem. Instead of putting them into one single column, we convert the centre information to K columns, with each row containing only 1 and 0, indicating this item belongs or doesn't belong to this category, respectively. For instance, if we have K=3 centres, we convert them to a 3x3 matrix as \{[1,0,0];[0,1,0];[0,0,1]\}. Some users choose to use K-1 columns. However, we highly recommend not to do this. The variations associated with the omitted category won't be removed by GLM.    

The last empty field can be used to enter `Regression targets'  (Figure \ref{Fig2.3}). This option allows the users to introduce a real number per subject to be used later for regression if that is the case. As mentioned above, this is the only way of entering the data and regression values when doing \textit{Regression} models.

Before Click 'Save', if users want to modify covariates and/or targets, simply right-click Modalities/runs space. In v2.1, we don't show all the input values anymore, especially when the numbers of covariates and targets are large, but use 'Entered' to indicate that values have been filled by users before (Figure \ref{covTargetsDis}).

\begin{figure}[!h]
	\begin{center}
		\includegraphics[height=3in]{images/covTargetsDis.png}
		\caption{Changed display of covariates and regression targets if users want to modify them. We have changed how we display the input covariates and targets. Instead of showing all the values, we show 'Entered' to indicate the fields are filled by users before.}
		\label{covTargetsDis}
	\end{center}
\end{figure}
