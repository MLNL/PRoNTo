...
When selecting by scans, the user can introduce `Covariates', i.e. a variable that covaries with the data (subjects) but of no interest to the subsequent analyses. This option is functional in version 2.1 of PRoNTo. It requires the input of a matrix, with one row per image/subject. This matrix can either be entered as a Matlab command in the editable box, or the full path to a .mat containing the matrix should be provided (matrix named  `R'). This last option is recommended to input a matrix (i.e. more than one covariate). The order of covariates should match the order the imaging files are selected. Before entering covariates in PRoNTo, we recommend users to firstly check that their covariates do not correlate with the targets as this could lead to biases (positive or negative) in the obtained results \cite{Miller2001}. 
In addition, if users have categorical covariates, such as genders, scanner centres, handness et al., we recommend one-hot encoding before constructing the covariate matrix. One-hot encoding represents each value of a variable (for example `center1', `center2') as a boolean `True/False' variable. The obtained variables are then concatenated. This process ensures that all values of the categorical variable are treated with the same importance. This wouldn't be the case if mapping the categorical values to integers, e.g. `center1' to 1 and `center2' to 2. In this last example, we'd assume that `center2' is \texit{bigger} than `center1' although there is no reason to encode an ordinal relationship between `center1' and `center2'. Here is an example code to perform one-hot encoding for an illustration covariate variable `cov': \linebreak
...
If multiple variables need to be encoded, this process should be repeated and all obtained matrices concatenated to obtain a $\# images \times \# variables$ matrix. Please note: a variant of this scheme ignores the last value as it is implicitly encoded by $[0,...,0]$. In the case of removing confounds, this is however not appropriate.    
