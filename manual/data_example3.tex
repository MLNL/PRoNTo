...
\begin{itemize}
	...
	\item Select the `Optimize hyper-parameter' tick box and leave as default or define an array with the hyper-parameter values that you wish to test. Note that if you leave it empty, PRoNTo will use an array with default hyper-parameter values;
	...
	\item In the `Data operations' box, select `Mean centre features using training data' option. Then, the `Specify model' window should look similar to the one in Figure \ref{fig:specifyModelmkl};
