...
\item \textbf{Summarizing weights per region:} Starting from PRoNTo v2.0, weights can be summarized within regions of interest as defined by an atlas (user-specified). For each region, a normalized contribution can be defined, and those contributions can then be ranked in descending order. If summarizing weights by using an atlas is desired, this atlas should be used as a second level mask in the 'feature set' step. Otherwise, any voxels used as features in the model but not present in the atlas will be summarized in one extra region referred to as 'other' that will not correspond to any anatomical region. In this case, users will see one extra region in the ROI label column in a table. 
\end{itemize}
