...
\begin{itemize}
	...
	\item In the `Covariates' field, type the covariates you want to regress out. Alternatively, you can save all the covariates in a matrix named {\tt R} and input the full path of this matrix to `Covariates'. Each covariate should be stored in a column, hence if there is only one covariate, {\tt R} is a column vector;if there are multiple variables, {\tt R} will be a matrix containing multiple columns where each column is a covariate. In this example, we input the full path of a matrix where gender and age are the chosen confounding factors (Figure \ref{fig:covariates});
