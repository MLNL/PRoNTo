...
%    args     - GMKL arguments (typicall the SVM soft-margin C and the sparsity v)
% Output:
...
% Written by J. Schrouff and J. Mourao-Miranda 

% Define default options for this machine
% -------------------------------------------------------------------------
% Based on the 'demo_MKL_L12.m' By Haiqin Yang
...
% Soft-margin and sparsity parameters
options.C = args(1);
C_opt = args(1);
...
if SANITYCHECK==true
    ...
    if ~isnumeric(args)
        error('prt_machine_GMKL_cla:MKLargsNotNumber',['Error: GMKL'...
    ...
    nC=numel(uTL);
    if nC>2
        error('prt_machine_GMKL_cla:problemNotBinary',['Error:'...
...
[beta,alpha_sv,b,pos] = GMKL_Level_L12(ktrain,tr_targets,C_opt,options,verbose);
